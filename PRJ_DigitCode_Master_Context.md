...
*   **Q3 (Count Demons):** Chọn 1 Hàng hoặc 1 Cột -> Trả về số lượng vạch LED đang sáng ("demons") thực tế trong nhóm đó. Nếu số lượng bằng Max LED -> Khóa và sáng vĩnh viễn toàn bộ vạch trong nhóm.
*   **Q4 (Check Danger):** Chọn 2 Hàng/Cột -> Trả về trạng thái có bị FULL quỷ (sáng hết vạch) hay không. Nút trên QML sẽ tự động đổi màu Xanh lá (Safe) hoặc Đỏ (Danger/Full).
...
*   Thiết kế sơ đồ nguyên lý (Schematic) & mạch PCB cho thiết bị phần cứng sử dụng **ESP32**.
...
*   Thiết kế mô hình vỏ hộp 3D (3D Casing) để in 3D.