...
**Bật thế nào.** Bằng extension `pg_cron`, qua Dashboard (Integrations) hoặc SQL. Không có bước nào yêu cầu thẻ tín dụng trong tài liệu. Trang tài liệu không nêu yêu cầu thẻ.
...
→ **Vậy `'30 seconds'` là cú pháp hợp lệ và Supabase tuyên bố hỗ trợ nó.** Đây là cơ chế duy nhất trong khảo sát có granularity dưới một phút.
...
→ Về mặt quota, `pg_cron` chạy SQL nội bộ **thật sự miễn phí**. Nó không đụng vào bất kỳ quota nào của Supabase Free trừ khi nó tạo ra thay đổi mà client đang nghe.
...
Đây là danh sách các khoảng trống thật sự — đã đọc trang gốc và không tìm thấy tuyên bố. Mỗi mục là một chỗ **không được phép suy đoán khi thiết kế**.
